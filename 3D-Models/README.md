...
You will also likely need to progrssively sand off the top edge of the **CB-Float-Actuator.stl** model to fine tune it to the break point of your optical switch. There's no way to make a one size fits all version of this actuator because all optical switches are not created equal. Remove little by little until the switch isn't tripped in the actuator's resting position.
