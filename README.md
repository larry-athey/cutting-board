...
Once a jar is full, the float will raise and break the light beam of an optical switch. This tells the ESP32 to raise the arm, advance 45 degrees to the next jar position, then lower the arm again. The other optical switch is on the linear actuator frame itself to detect when it is at it's zero position. An actuator calibration procedure runs at every startup to find the zero position of the linear actuator.
